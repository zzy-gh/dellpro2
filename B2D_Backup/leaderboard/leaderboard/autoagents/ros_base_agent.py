...
class ROSBaseAgent(AutonomousAgent):
    ...
    def _vehicle_control_cmd_callback(self, control_msg):
        ...
        # Drain any stale entry so the newest control is never silently dropped.
        # With async inference pipelines (e.g. remote Orin), commands arrive faster
        # than run_step can consume them; always keeping the latest is correct.
        try:
            self._control_queue.get_nowait()
        except queue.Empty:
            pass
        self._control_queue.put_nowait((carla_timestamp, control))
    ...
    def run_step(self, _, timestamp):
        ...
        try:
            control_timestamp, control = self._control_queue.get(True, 5.0)
            self._last_control = control
        except queue.Empty:
            control = getattr(self, '_last_control', carla.VehicleControl())
