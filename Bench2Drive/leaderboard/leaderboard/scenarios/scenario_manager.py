...
class ScenarioManager(object):
    ...
    def run_scenario(self):
        ...
        # Warm up the odometry buffer before the agent loop starts.
        # Bare world.tick() in setup_sensors does not complete the full synchronous
        # pipeline (GameTime + CarlaDataProvider updates) that the ROS bridge needs to
        # fire sensor callbacks, so we do the warm-up here with proper full ticks but
        # without calling the agent.
        self._odometry_warmup()

        while self._running:
            self._tick_scenario()

    # Seconds of simulation time to tick before the first agent call so the VLA
    # odometry buffer is full.
    ODOMETRY_WARMUP_SECONDS = 3.0
    # Wall-clock sleep after each warm-up tick so carla_ros_bridge's sensor
    # callbacks (which run in a background thread) have time to fire and publish
    # to ROS2 before the next tick is triggered.
    ODOMETRY_WARMUP_SLEEP_S = 0.5

    def _odometry_warmup(self):
        world = CarlaDataProvider.get_world()
        delta = world.get_settings().fixed_delta_seconds or 0.05
        warmup_ticks = max(1, int(self.ODOMETRY_WARMUP_SECONDS / delta))
        print("[ScenarioManager] Warming up odometry buffer: {} ticks ({:.1f}s sim time, ~{:.0f}s wall time)".format(
            warmup_ticks, warmup_ticks * delta, warmup_ticks * self.ODOMETRY_WARMUP_SLEEP_S))
        for _ in range(warmup_ticks):
            world.tick(self._timeout)
            timestamp = world.get_snapshot().timestamp
            GameTime.on_carla_tick(timestamp)
            CarlaDataProvider.on_carla_tick()
            self._watchdog.update()
            time.sleep(self.ODOMETRY_WARMUP_SLEEP_S)
